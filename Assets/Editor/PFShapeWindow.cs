...
namespace mattatz.ProceduralFlower
{
	...
	public class PFShapeWindow : EditorWindow
	{
		...
		[MenuItem ("ProceduralFlower/Shape Editor")]
		public static void Open ()
		{
			...
			window.titleContent = new GUIContent (" PF Shape Editor", icon);
			window.Focus ();
		}
		...
		void DrawHeader ()
		{
			...
			if (shape) {
				GUI.Label (new Rect (455f, 5f, 80f, 25f), shape.name, style);
			} else {
				GUI.Label (new Rect (455f, 5f, 80f, 25f), "Select shape asset", style);
			}

			if (shape && GUI.Button (new Rect (455f, 30f, 80f, 25f), "Save")) {
				Apply ();
			}

			if (shape && GUI.Button (new Rect (455f, 60f, 80f, 25f), "Delete point")) {
				if (selected >= 0) {
					Vector2 thisPoint = points [selected];
					points.Remove (thisPoint);
				}
			}
			if (shape && GUI.Button (new Rect (455f, 90f, 80f, 25f), "Clear")) {
				points.Clear ();
			}
		}
		...
		void OnGUI ()
		{
			...
			size.x = size.y = Mathf.Min (Screen.width, Screen.height / 3);
